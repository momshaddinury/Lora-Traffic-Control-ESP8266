...
int8_t SX1278::getPacket(uint32_t wait)
{
	...
	if( p_received == true )
	{
		...
		if( _modem == LORA )
		{
			...
			#if (SX1278_debug_mode > 0)
				Serial.printf("_payloadlength %d", _payloadlength);
				Serial.println();
			#endif
		}
		...
		{
			#if (SX1278_debug_mode > 0)
				Serial.println(F("Corrupted packet, length must be less than 256 and greater than 0"));
			#endif
		}
		else
		{
...
byte SX1278::setLoraToSleep(){
    byte state=readRegister(REG_OP_MODE);
	if( _modem == LORA ){

        writeRegister(REG_OP_MODE,LORA_SLEEP_MODE);
        state = readRegister(REG_OP_MODE);

			Serial.println(F("## Lora Set to sleep ##"));
			Serial.println("REG_OP_MODE current value"+String(state));


	}
	return state;
}
byte SX1278::readREGOPMODE(){
